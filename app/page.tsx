...
export default function Page() {
  ...
  function saveSettings(patch: Partial<Settings>) {
    ...
    setSettings(next); void pushSettings(next);
  }
...
');
  ...
  async function copyPrompt() {
    try { await navigator.clipboard.writeText(endOfDayPrompt); alert('Prompt copied'); } catch {}
  }
  ...
  if (!userEmail) {
    return (
      <div className="card space-y-3">
        ...
        <p className="text-sm text-neutral-600">PeakForm now uses Supabase only. Please <a className="underline" href="/login">log in with a magic link</a> to start logging.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="card">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          ...
          <div>
            ...
            <input className="input" type="number" value={settings.calorieTarget}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="card space-y-2">
          ...
          <div className="text-sm">Status: <span className="font-medium capitalize">{totals.balance}</span> ({totals.deficit > 0 ? `${fmtNum(totals.deficit)} kcal below target` : totals.deficit < 0 ? `${fmtNum(Math.abs(totals.deficit))} kcal above target` : 'on target'})</div>
        </div>
...
function range(l:string,p:string,v:any,on:(p:string,v:any)=>void){return(<div className="select-none">{label(`${l} ${(v||'—')}/5`)}<input type="range" min={1} max={5} step={1} value={v||3} onChange={(e)=>on(p,e.target.value)} className="w-full"/></div>)}
...
import { useEffect, useMemo, useState } from 'react';
import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, ReferenceLine } from 'recharts';
import { emptyEntry, defaultSettings, readLocalEntries, readLocalSettings, writeLocalEntries, writeLocalSettings, pushEntry, pushSettings, pullCloud, getUser } from '@/lib/storage';
import type { Entry, Settings } from '@/lib/types';
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (user) {
        const cloud = await pullCloud();
        if (cloud) {
          // merge: cloud wins per date; keep any local-only days
          const merged = { ...readLocalEntries(), ...cloud.entries };
          setEntries(merged); writeLocalEntries(merged);
          if (cloud.settings) { setSettings(cloud.settings); writeLocalSettings(cloud.settings); }
        }
      }
    })();
  }, []);
  ...
  function update(path: string, value: any) {
    setEntries(prev => {
      ...
      obj[keys.at(-1)!] = value;
      const out = { ...prev, [date]: next };
      writeLocalEntries(out);
      // push to cloud if logged in (fire & forget)
      void pushEntry(date, next);
      return out;
    });
  }
  ...
  function saveSettings(patch: Partial<Settings>) {
    const next = { ...settings, ...patch } as Settings;
    setSettings(next); writeLocalSettings(next); void pushSettings(next);
  }
...
');
  ...
  function exportJSON() {
    ...
    a.href = url; a.download = `peakform-${new Date().toISOString().slice(0,10)}.json`; a.click(); URL.revokeObjectURL(url);
  }
  function importJSON(file: File) {
    const reader = new FileReader(); reader.onload = () => {
      try {
        const parsed = JSON.parse(String(reader.result));
        if (parsed.entries) { setEntries(parsed.entries); writeLocalEntries(parsed.entries); }
        if (parsed.settings) { setSettings(parsed.settings); writeLocalSettings(parsed.settings); }
    ...
    <div className="space-y-6">
      <div className="card">
        <div className="grid-4 items-end">
          ...
          <div className="text-sm self-center text-neutral-600">{userEmail ? `Signed in as ${userEmail}` : 'Not signed in (data stored locally)'}</div>
          <div className="flex gap-2 justify-end">
            <button className="btn" onClick={exportJSON}>Export</button>
            <label className="btn cursor-pointer">Import<input type="file" className="hidden" accept="application/json" onChange={(e)=> e.target.files?.[0] && importJSON(e.target.files[0])}/></label>
          </div>
        </div>
      </div>
