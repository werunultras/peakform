...
const fmtNum = (v: number) =>
  Number.isFinite(v) ? new Intl.NumberFormat().format(v) : '—';
const toNum = (v: any) => {
...
export default function Page() {
  ...
  const totals = useMemo(() => {
    ...
    const deficit = settings.calorieTarget
      ? settings.calorieTarget - cals
      : 0;
    const balance =
      deficit === 0 ? 'balance' : deficit > 0 ? 'deficit' : 'surplus';
    return {
      calories: cals,
  ...
  // streak = consecutive days with nutrition calories > 0
  ...
  const endOfDayPrompt = useMemo(() => {
    ...
    const lines = [
      ...
      r.distanceKm || r.durationMin || r.pace
        ? `• Run: ${fmt(r.distanceKm, 'km')} · ${fmt(
            r.durationMin,
            'min'
          )} · ${fmt(r.pace, 'pace')}`
        : null,
      r.hrAvg || r.hrMax
        ? `  HR: ${fmt(r.hrAvg, 'avg')} / ${fmt(r.hrMax, 'max')}`
        : null,
      r.cadence || r.strideM
        ? `  Cadence/Stride: ${fmt(r.cadence, 'spm')} · ${fmt(
            r.strideM,
            'm'
          )}`
        : null,
      r.elevUp || r.elevDown
        ? `  Elevation: +${fmt(r.elevUp, 'm')} / −${fmt(r.elevDown, 'm')}`
        : null,
      r.calories || r.sweatLossL
        ? `  Calories: ${fmt(r.calories, 'kcal')} · Est. sweat loss ~${fmt(
            r.sweatLossL,
            'L'
          )}`
        : null,
      ...
      'Nutrition',
      `• Calories: ${n.calories || '—'}`,
      `• Macros: Carbs ${n.carbsG || '—'} g | Protein ${
        n.proteinG || '—'
      } g | Fat ${n.fatG || '—'} g | Fibre ${n.fibreG || '—'} g`,
      '',
      `Mindset — Mood ${m.mood || '—'}/5 · Stress ${
        m.stress || '—'
      }/5 · Sleep Q${m.sleepQuality || '—'}/5`,
    ].filter(Boolean);
  ...
  // txt import
  function parseDiaryTxt(txt: string): {
    date: string;
    entry: Entry;
    ...
    const strength = {
      ...
      weightLbs: map['STRENGTH_WEIGHT_LBS'],
    };
    const nutrition = {
    ...
    const calorieTarget = map['CALORIE_TARGET']
      ? Number(map['CALORIE_TARGET'])
      : undefined;
  ...
  async function handleImportTxt(file: File) {
    ...
    const { date: d, entry: imported, calorieTarget } =
      parseDiaryTxt(txt);
    setEntries((prev) => ({
      ...prev,
      [d]: { ...(prev[d] ?? emptyEntry(d)), ...imported },
    }));
    ...
    return (
      <div className="card space-y-3">
        ...
        <p className="text-sm text-neutral-600">
          PeakForm uses Supabase only. Please{' '}
    ...
    <div className="space-y-6">
      ...
      <div className="card">
        ...
        <div className="flex items-center gap-3 text-sm">
          ...
          <div className="text-neutral-600">
            Format: KEY=VALUE per line (e.g., DATE=2025-08-30, DIST_KM=10,
            CALORIE_TARGET=2600 …).
          </div>
        </div>
      </div>
      ...
      <div className="card space-y-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {num(
            'Distance (km)',
            'workout.run.distanceKm',
            entry.workout.run.distanceKm,
            update
          )}
          {num(
            'Duration (min)',
            'workout.run.durationMin',
            entry.workout.run.durationMin,
            update
          )}
          {text(
            'Pace (e.g. 5:30/km)',
            'workout.run.pace',
            entry.workout.run.pace,
            update
          )}
